...
$('.spin-button').on('click', event => {
  newRoundWheel.spin();
  if (newRoundWheel.currentSpin !== 'BANKRUPT' && newRoundWheel.currentSpin !== 'LOSE A TURN') {
    $('.turn-option-div').addClass('hidden');
    $('.consonant-board').removeClass('hidden');
  } else if (newRoundWheel.currentSpin === 'BANKRUPT') {
    currentGame.players[currentGame.currentPlayer].resetRoundScore();
    domUpdates.updateRoundScoreOnDom(currentGame.currentPlayer, 0);
