class Game {
  ...
  generatePuzzleBoard(currentRound) {
    this.currentAnswer = currentGame.puzzles[currentRound].correct_answer;
  ...
  checkPlayerAnswer(guess) {
    if (guess.toLowerCase() === this.currentAnswer.toLowerCase()) {
      ...
      //current player and call currentplayer.sumTotalScore
      this.startNewRound();
    } else {
  ...
  updateCurrentPlayer() {
    // update the currentplayer.isTurn property by calling currentplayer.updateTurn();
    this.players[this.currentPlayer].updateTurn();
    // check if the current player is at index 2, if yes, reset current player to 0
    // if no, take the current player and increment it.
    // update the next current player isTurn
    this.currentPlayer++
