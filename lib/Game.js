class Game {
    ...
    this.answerLettersArray.forEach((word, i) => {
      if (numberOfWords === 1) {
        word.forEach((character) => {
          $(`.letter-card-div-${rowTwo}`).text(character).addClass('superhidden').attr('data-value', character);
          rowTwo++;
        }) 
      } else if (numberOfWords > 1) {
        word.forEach((character) => {
          if (i === 0) {
            $(`.letter-card-div-${rowOne}`).text(character).addClass('superhidden').attr('data-value', character);
            rowOne++;
          } else if (i === 1) {
            $(`.letter-card-div-${rowTwo}`).text(character).addClass('superhidden').attr('data-value', character);
            rowTwo++;
          } else if (i === 2) {
            $(`.letter-card-div-${rowThree}`).text(character).addClass('superhidden').attr('data-value', character);
            rowThree++;
          } else if (i === 3) {
            $(`.letter-card-div-${rowFour}`).text(character).addClass('superhidden').attr('data-value', character);
            rowFour++;
          }
        }) 
      } 
  ...
  checkLetterAnswer(letter) {
    ...
    if (this.currentAnswer.includes(letter)) {
      ...
      $(`[data-value = ${letter}]`).removeClass('tomato');


    //filter
      // letter.removeClass('tomato');
      // console.log(clickedLetter);
      //add it as a guessed letter
      //show on puzzle board
    } else {
